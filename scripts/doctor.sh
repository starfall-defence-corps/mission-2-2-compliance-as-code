...
MISSION="Mission 2.2: Compliance as Code"
...
if [ "${SDC_RUNNING:-0}" -gt 0 ]; then
    ...
    warn "SDC lab containers already running: ${THIS_LAB}" "If these belong to another mission, run 'make destroy' there first — this lab uses ports 2221-2223, 2251"
else
    PORTS_BUSY=""
    for port in 2221 2222 2223 2251; do
    ...
    if [ -n "$PORTS_BUSY" ]; then
        fail "Ports 2221-2223, 2251 free" "Port(s)${PORTS_BUSY} in use — another mission lab or service is listening. Run 'make destroy' in the other mission, or free the port"
    else
        pass "Ports 2221-2223, 2251 free"
    fi
fi
